...
use crate::{
    ...
    IssueKind, IssueSeverity, ParseHardError, ParseIssue, ParsedSave, Save, SaveMeta, SaveSummary,
    Strictness,
};
...
///
/// Equivalent to [`summarize_with_strictness`] with [`Strictness::Lax`].
pub fn summarize(bytes: &[u8]) -> Result<SaveSummary, ParseHardError> {
    summarize_with_strictness(bytes, Strictness::Lax)
}

/// Summarize top-level header and character fields for list views.
pub fn summarize_with_strictness(
    bytes: &[u8],
    strictness: Strictness,
    ...
    match decode_character_for_format(selected_layout.format_id(), &bytes[character_range.clone()]) {
        Ok(character) => {
            let expansion_type =
            ...
            let title = crate::character::title_for_progression_d2r(
                ...
                character.status.is_hardcore(),
            )
            .map(str::to_string);
            summary.expansion_type = Some(expansion_type);
            summary.name = Some(character.name.clone());
            summary.class = Some(character.class);
            summary.level = Some(character.level);
            summary.title = title;
        }
        Err(parse_error) => {
            push_issue(
