...
impl From<u32> for FixedPointStat{
    fn from(fixed_point_number: u32) -> FixedPointStat {
        ...
        FixedPointStat{integer:integer, fraction:fraction}
    }
}
...
fn write_u8(byte_vector : &mut Vec<u8>, byte_position: &mut BytePosition, bits_source : u8, bits_count : usize) -> () {
    ...
    loop {
        ...
        if byte_position.current_bit == 0 && bits_count == 8 {
        ...
        byte_vector[byte_position.current_byte].set_bit_range(byte_position.current_bit..(byte_position.current_bit+bits_can_write_in_byte), bits_source.bit_range(bit_index..(bit_index+bits_can_write_in_byte)));
...
fn write_u32(byte_vector : &mut Vec<u8>, byte_position: &mut BytePosition, bits_source : u32, bits_count : usize) -> () {
    let mut bits_left_to_write : usize = bits_count;
    ...
    loop {
        ...
        println!("Writing {bits_can_write:?} bits of byte: {0:#010b}", byte_source[byte_source_current]);
...
impl From<Attributes> for Vec<u8>{
    fn from(stats: Attributes) -> Vec<u8> {
        let mut byte_vector : Vec<u8> = vec!(0);
        let mut byte_position = BytePosition::default();

        for header in 0..STAT_NUMBER{
            let header_as_u32 = header as u32;
            let mut bits_left_to_insert = STAT_HEADER_LENGTH;
...
pub fn parse_attributes_with_position (byte_vector : &Vec<u8>, byte_position: &mut BytePosition) -> Attributes {
...
mod tests {
    ...
    fn test_parse_attributes_1() {
        ...
        let expected_stats = Attributes {
            ...
            stat_points_left: 0,
            skill_points_left: 0,
            life_current: FixedPointStat{integer: 55, fraction: 0},
            life_base: FixedPointStat{integer: 55, fraction: 0},
            mana_current: FixedPointStat{integer: 10, fraction: 0},
            mana_base: FixedPointStat{integer: 10, fraction: 0},
            stamina_current: FixedPointStat{integer: 92, fraction: 0},
            stamina_base: FixedPointStat{integer: 92, fraction: 0},
            level: 1,
            experience: 0,
            gold_inventory: 0,
            gold_stash: 0
    ...
    fn test_parse_attributes_2() {
        // Level 92 sorceress
        let bytes : Vec<u8> = vec!(0x00, 0x38, 0x09, 0x30, 0x82, 0x80,
            0x11, 0x06, 0x10, 0x65, 0x00, 0x80, 0x9D, 0x1C, 0x00, 0x98,
            0x19, 0x08, 0x98, 0x2A, 0x45, 0x02, 0x80, 0x6C, 0xA0, 0x00,
            0xA0, 0x44, 0x2C, 0x00, 0xF8, 0x0E, 0x0C, 0xB8, 0x0D, 0xDE,
            0xA3, 0xD1, 0xF2, 0x1E, 0x30, 0xCE, 0x02, 0xF8, 0x0F, 0x69,
            0x66, 0x00, 0x01, 0x00, 0x14, 0x01, 0x00, 0x01, 0x01, 0x01,
            0x11, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
        ...
        let expected_stats = Attributes {
            ...
            stat_points_left: 0,
            skill_points_left: 0,
            life_current: FixedPointStat{integer: 1260, fraction: 0},
            life_base: FixedPointStat{integer: 819, fraction: 0},
            mana_current: FixedPointStat{integer: 661, fraction:76},
            mana_base: FixedPointStat{integer: 217, fraction: 0},
            stamina_current: FixedPointStat{integer: 549, fraction: 0},
            stamina_base: FixedPointStat{integer: 479, fraction: 0},
            level: 92,
