...
mod tests {
    ...
    fn test_named_lookup_d2r_roundtrip() {
        ...
        assert_eq!(
            d2r_skill_name(Class::Warlock, 22).expect("index 22 should exist"),
            "Miasma Bolt"
        );
    ...
    fn test_named_skill_set_get() {
        let mut skill_points = SkillPoints::default();
        skill_points
            .set_by_name_d2r(Class::Sorceress, "Teleport", 20)
            .expect("teleport should exist");
        assert_eq!(
            skill_points
                .get_by_name_d2r(Class::Sorceress, "teleport")
                .expect("teleport should exist"),
            20
        );
    }
